...
const NotificationPanel = ({ onClose }: NotificationPanelProps) => {
  ...
  return (
    <div className="absolute right-0 top-12 w-80 sm:w-96 bg-white rounded-xl shadow-xl border border-gray-200 z-50 max-h-96 overflow-hidden animate-fade-in animate-scale-in transform origin-top-right">
      ...
      <div className="overflow-y-auto max-h-80 scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100">
        {isLoading ? (
              ...
              >
                <div className="flex items-start gap-3">
                  <div className="flex-shrink-0 mt-0.5">
                    <div className={`p-2 rounded-full ${!notification.is_read ? 'bg-teal-100' : 'bg-gray-100'}`}>
                      <MessageCircle size={14} className={!notification.is_read ? 'text-teal-600' : 'text-gray-500'} />
                    </div>
                  </div>
          ...
          <div className="text-center py-12 animate-fade-in">
            ...
            <p className="text-gray-500 text-sm font-medium mb-1">No notifications yet</p>
            <p className="text-gray-400 text-xs">You'll see notifications here when someone interacts with your posts</p>
          </div>
        )}
      </div>
    </div>
